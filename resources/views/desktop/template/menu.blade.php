        ...
        <div class="flex justify-between /items-center /py-[10px]">
            <div class="lg:hidden relative flex items-center">
                ...
                <div class="block lg:hidden w-[72px] 400:w-[99px]">
                    <a id="menuToggle" class="flex flex-col items-start justify-center py-1 ml-0 text-black md:py-2 header-menu-btn menu-footer-item h-[60px] rounded-full cursor-pointer">
                        <svg class="w-[25px] sm:w-[40px] h-auto" width="40px" height="40px" viewBox="0 0 24 24" clip-rule="evenodd" fill-rule="evenodd" stroke-linejoin="round" stroke-miterlimit="2" xmlns="http://www.w3.org/2000/svg">
                            <path d="m13 16.745c0-.414-.336-.75-.75-.75h-9.5c-.414 0-.75.336-.75.75s.336.75.75.75h9.5c.414 0 .75-.336.75-.75zm9-5c0-.414-.336-.75-.75-.75h-18.5c-.414 0-.75.336-.75.75s.336.75.75.75h18.5c.414 0 .75-.336.75-.75zm-4-5c0-.414-.336-.75-.75-.75h-14.5c-.414 0-.75.336-.75.75s.336.75.75.75h14.5c.414 0 .75-.336.75-.75z" fill="#194569"/>
                        </svg>
                    </a>
                </div>
                <!-- Modal Menu -->
                <div id="modal-menu" class="fixed inset-0 z-50 hidden bg-gray-800 bg-opacity-75 opacity-0 transition-opacity duration-300 ease-in-out">
                    <div class="bg-white w-3/4 md:w-1/3 h-full shadow-lg transform transition-transform duration-300 ease-in-out">
                        <!-- Close Button -->
                        ...
                        <!-- Search Bar -->
                        <div class="p-4">
                            <input type="text" id="keyword_mobile" placeholder="Search..." class="w-full rounded-full p-2 border border-gray-300 focus:outline-none">
            ...
            <!-- logo -->
            <div class="logo">
                <a href="{{route ('home')}}" class="flex justify-center"> <img src="../img/Logo (1).png" alt="" class="/w-[70%] sm:w-full"></a>
            </div>
            <!-- menu -->
            <div class="menu_des hidden lg:flex gap-8 items-center ">
                <div class="menu">
                    <ul class="flex gap-12  items-center">
                        ...
                        <li><a href="{{route ('about')}}" class="text-black font-el font-semibold text-[13px] 2xl:text-base hover:text-cmain flex items-center gap-1">Giới Thiệu</a></li>
                        ...
                        <li class="relative group ">
                            ...
                            <ul class="absolute top-full bg-white invisible opacity-0 group-hover:visible transition-all duration-300 group-hover:opacity-100 w-[200px]">
                                @foreach ($categories as $cate)
                                    <li class="px-4 py-2 "><a href="{{route('listcate', ['name' => $cate->name])}}" class="text-black  font-el font-semibold text-[13px] 2xl:text-base hover:text-cmain ">{{$cate->name}}</a></li>
                                @endforeach
                            </ul>
                        </li>

                        <li><a href="{{route ('blog')}}" class="text-black font-el font-semibold text-[13px] 2xl:text-base hover:text-cmain flex items-center gap-1">Tin tức</a></li>
                        <li><a href="{{route ('contact')}}" class="text-white font-el font-semibold text-[13px] 2xl:text-base px-5 py-3 rounded-[10px] bg-[#194569] flex items-center gap-1">Liên hệ ngay</a></li>
                    </ul>
                </div>
                <!-- //////////////////////// -->
                <div class="sgv flex gap-6 items-center ">
                    ...
                    <div class="cart fix_cart_count relative">
                        ...
                        @else
                            <a href="{{ route('login') }}"><svg width="25" height="24" viewBox="0 0 25 24"
                        ...
                        <span
                            class="count-cart ajax-count-cart text-white bg-cmain min-w-[20px] min-h-[20px] flex items-center justify-center rounded-[20px] text-[9px] absolute -top-2 -right-3 leading-[16px]">{{-- {{countCart(Auth::user()->id)}} --}}10</span>
                    </div>
                    <!-- search -->
                    <div class="search">
                        <div class="relative inline-block">
                            ...
                            <div class="cursor-pointer" onclick="toggleSearch()">
                                ...
                                <path d="M20.5 20L22.5 22" stroke="#454545" stroke-width="2" stroke-linecap="round"/>
                                </svg>
                            </div>
                            <!-- Search Box -->
                            <form action="{{ route('search') }}" method="GET" class="">
                                @csrf
                            <div id="searchBox" class="absolute top-full right-0 mt-10 hidden bg-white p-4 shadow-lg rounded-lg">
                                <input id="keyword" type="text" name="query" value="{{ request('query') }}" class="border border-gray-300 p-2 rounded-md focus:ring-2 focus:ring-orange-500 focus:outline-none mb-2" placeholder="Tìm kiếm..." />
                            </div>
                            </form>
                        </div>
                    </div>
                    <div class="account group relative">
                            ...
                            <ul class="absolute top-[65px] right-0 bg-white invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-all duration-300 w-[200px] shadow-lg rounded-md">
                                @if (!Auth::check())
                                    <li class="px-4 py-2">
                                        <a href="{{ route('login') }}" class="text-black font-el font-semibold text-[13px] 2xl:text-base hover:text-cmain">
                                            Đăng nhập
                                        </a>
                                    </li>
                                    @if (Route::has('register'))
                                        <li class="px-4 py-2">
                                            <a href="{{ route('register') }}" class="text-black font-el font-semibold text-[13px] 2xl:text-base hover:text-cmain">
                                                Đăng ký
                                            </a>
                                        </li>
                                    @endif
                                @else
                                    <li class="px-4 py-2">
                                        <span class="text-black font-el font-semibold text-[13px] 2xl:text-base">Xin chào, {{ Auth::user()->fullname }}</span>
                                    </li>
                                    <li class="px-4 py-2">
                                        <form method="POST" action="{{ route('logout') }}">
                                            @csrf
                                            <button type="submit" class="text-black font-el font-semibold text-[13px] 2xl:text-base hover:text-cmain">
                                                {{ __('Đăng xuất') }}
                                            </button>
                                        </form>
                                    </li>
            ...
            <div class="sgv flex lg:hidden gap-6 /items-center justify-center items-center">
                ...
                <div class="search">
                    <div class="relative inline-block">
                        ...
                        <!-- Search Box -->
                        <div id="searchBox" class="absolute top-full right-0 mt-7 hidden bg-white p-4 shadow-lg rounded-lg">
                            <input id="keyword" type="text" class="border border-gray-300 p-2 rounded-md focus:ring-2 focus:ring-orange-500 focus:outline-none mb-2" placeholder="Tìm kiếm..." />
                        </div>
                    </div>
                </div>
            </div>
